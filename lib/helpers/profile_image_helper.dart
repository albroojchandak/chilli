...
class ProfileImageHelper {
  // Female Avatars - Curated Pinterest List
  ...
  // Male Avatars - Curated Pinterest List
  ...
  static String getRandomAvatar(String gender) {
    final random = Random();
    final isFemale = gender.toLowerCase() == 'female';
    final avatarList = isFemale ? femaleAvatars : maleAvatars;
    return avatarList[random.nextInt(avatarList.length)];
  }

  /// Check if an avatar URL is old (Dicebear) and needs replacement
  static bool isOldAvatar(String? avatarUrl) {
    if (avatarUrl == null || avatarUrl.isEmpty) return true;
    return avatarUrl.contains('dicebear.com');
  }

  /// Get avatar URL - returns existing if valid, or generates new one
  static String getOrGenerateAvatar(String? currentAvatar, String gender) {
    if (isOldAvatar(currentAvatar)) {

...
class RealtimeDbService {
  ...
  Stream<List<UserData>> getAllUsers({String? targetGender}) {
    ...
    void emitMergedList() {
      ...
      final List<UserData> mergedUsers = [];
      ...
      profileMap.forEach((key, val) {
        if (val is Map) {
          ...
          if (data['a'] != null) {
            ...
            if (ProfileImageHelper.isOldAvatar(avatar)) {
              final gender = data['g']?.toString() ?? 'male';
              data['avatarUrl'] = ProfileImageHelper.getRandomAvatar(gender);
  ...
  Future<List<UserData>> getUsersList({String? targetGender}) async {
    try {

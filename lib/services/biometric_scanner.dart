...
class BiometricScanner {
  ...
  // For Login/Registration Screen (using camera package)
  Future<Map<String, dynamic>> analyzeCameraImage(
    CameraImage image,
  ...
  // --- For Call Screen (using flutter_webrtc) ---
  Function(bool hasFace)? onFaceDetected;
  Timer? _dummyDetectionTimer;

  Future<void> initialize(MediaStream stream) async {
    _dummyDetectionTimer?.cancel();
    _dummyDetectionTimer = Timer.periodic(const Duration(seconds: 2), (timer) {

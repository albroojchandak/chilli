...
import 'package:chilli/core_services/face_recognition_service.dart';
...
class FaceVerifyPage extends StatefulWidget {
  ...
  const FaceVerifyPage({super.key, required this.onVerified, this.gender});
...
class _FaceVerifyPageState extends State<FaceVerifyPage>
  ...
  Future<void> _processImage(CameraImage image) async {
    _isProcessing = true;
    try {
      final result = await _FaceRecognitionService.analyzeCameraImage(
        image,
      ...
      if (faces.isNotEmpty) {
        ...
        for (var label in labels) {
          ...
          // Check for clear gender markers with high confidence
          if (confidence > 0.6) {
        ...
        if (targetGender == 'male') {
          // Male: Allow if not a dominant female detection
          genderMatch =
              !foundFemaleIndicator ||
              (foundFemaleIndicator && foundMaleIndicator);
        } else {
          // Female Verification Logic
          // ⚠️ Mandatory Block: If beard, mustache or "Man" label is detected, REJECT.
          if (foundMaleIndicator) {
            genderMatch = false;
          } else {
            // Heuristics Check (Liveness + Expression)
            ...
            // Female verification: Needs heuristics (to ensure it's a person)
            // and must NOT have any male indicators.
            genderMatch =
                hasSmile &&
                hasEyesOpen &&
                cheeksVisible &&

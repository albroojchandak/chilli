...
import '../core_services/push_sender_service.dart';
import '../core_services/http_service.dart'; // Add this
import '../core_services/face_recognition_service.dart';
import '../core_services/authentication_service.dart';
import '../data_models/gift_model.dart'; // Gift system models
...
class _VideoCallPageState extends State<VideoCallPage>
  ...
  final PushSenderService _fcmSender = PushSenderService();
  final AuthenticationService _AuthenticationService = AuthenticationService();
  final HttpService _HttpService =
      HttpService(); // Add after AuthenticationService
  ...
  // GiftModel system state
  bool _showGiftModelMenu = false;
  List<GiftModel> _availableGiftModels = [];
  final List<Map<String, dynamic>> _GiftModelHistory = [];
  GiftModel? _currentAnimatingGiftModel;
  ...
  Future<void> _initCallData() async {
    ...
    PushNotificationService().isInCall = true;
  ...
  Future<void> _initializeOutgoingFlow() async {
    try {
      ...
      // ✅ Setup GiftModel listener for outgoing flow as well
  ...
  Future<void> _initializeCall() async {
    try {
      ...
      // ✅ Setup GiftModel listener for incoming calls
      _setupGiftModelListener();
    } catch (e) {
      _showLocalEndMessage(
        'Error',
  ...
  void _loadAvailableGiftModels() {
    setState(() {
      _availableGiftModels = GiftModelCatalog.allGiftModels;
    });
    print(
      '🎁 Loaded ${_availableGiftModels.length} GiftModels in VideoCallPage',
    );
  }
  ...
  void _cleanup() {
    ...
    PushNotificationService().isInCall = false;
  ...
  @override
  void dispose() {
    print('VideoCallPage dispose called');
  ...
  Future<void> _sendGiftModel(GiftModel GiftModel) async {
    ...
    // Check token balance
    if (_currentTokens < GiftModel.cost) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Need ₹${GiftModel.cost} (You have ₹$_currentTokens)'),
    ...
    try {
      // Create GiftModel message
      final giftMessage = GiftModelMessage(
        GiftModelId: GiftModel.id,
        senderName: _currentUserName ?? 'User',
        senderAvatar: widget.callerAvatar,
        senderGender: _currentUserGender ?? 'male',
        timestamp: DateTime.now(),
        senderCost: GiftModel.cost,
        receiverReward: GiftModel.reward,
      );
      ...
      if (success) {
        ...
        await _HttpService.updateLocalCoins(GiftModel.cost, isDeduction: true);
        final newBalance = await _HttpService.getLocalCoins();
        ...
        _showGiftModelSentAnimation(GiftModel);

        // Broadcast the update
        HttpService.broadcastTokenUpdate(newBalance);
        print(
          '🎁 GiftModel sent: ${GiftModel.name} (${GiftModel.emoji}) - Cost: ${GiftModel.cost} tokens',
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to send GiftModel. Please try again.'),
            backgroundColor: Colors.red,
          ),
        );
      }
    } catch (e) {
      print('❌ Error sending GiftModel: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Error sending GiftModel'),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  void _handleGiftModelReceived(
    ...
    if (_currentUserGender == 'female') {
      ...
      await _HttpService.updateLocalCoins(GiftModel.reward, isDeduction: false);
      final newBalance = await _HttpService.getLocalCoins();
      ...
      HttpService.broadcastTokenUpdate(newBalance);
      print(
        '🎁 Reward received: +₹${GiftModel.reward}, New balance: ₹$newBalance',
      );
    }

    // Show GiftModel received animation
    _showGiftModelReceivedAnimation(GiftModel, GiftModelMsg);
  }

  void _showGiftModelSentAnimation(GiftModel GiftModel) {
    setState(() {
      _currentAnimatingGiftModel = GiftModel;
    ...
    Future.delayed(GiftModelAnimation.getDuration(GiftModel.animation), () {
      if (mounted) {
        setState(() {
  ...
  void _showGiftModelReceivedAnimation(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(
          children: [
            Text(GiftModel.emoji, style: const TextStyle(fontSize: 24)),
            const SizedBox(width: 8),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                ...
                children: [
                  Text(
                    '${GiftModelMsg.senderName} sent you a ${GiftModel.name}!',
                  ),
                  if (_currentUserGender == 'female')
                    Text(
                      '+₹${GiftModel.reward} earned',
                      style: const TextStyle(
  ...
  // Update _initializeOutgoingFlow or _initializeIncomingFlow to add GiftModel listener
  void _setupGiftModelListener() {
    _VideoCallService.onMessageReceived = (String message) {
      try {
        // Check if it's a GiftModel message
        if (message.contains('"type":"GiftModel"')) {
          final GiftModelMsg = GiftModelMessage.fromJson(message);
          final GiftModel = GiftModelCatalog.getGiftModelById(
            GiftModelMsg.GiftModelId,
          );
          if (GiftModel != null) {
            _handleGiftModelReceived(GiftModel, GiftModelMsg);
          }
        }
      } catch (e) {
  ...
  Widget _buildGiftModelMenu() {
    ...
    print(
      '🎁 Building GiftModel menu - GiftModels count: ${GiftModels.length}, connected: $_isConnected',
    );
  ...
  Widget _buildGiftModelAnimationOverlay() {
    ...
    final GiftModel = _currentAnimatingGiftModel!;
          ...
          child: Align(
            ...
            child: Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(GiftModel.emoji, style: const TextStyle(fontSize: 28)),

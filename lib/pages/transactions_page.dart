...
class TransactionsPage extends StatefulWidget {
  ...
  const TransactionsPage({Key? key, this.isWithdrawal = false})
    : super(key: key);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (statusOk && txnStatus == 'SUCCESS') {
          ...
          if (alreadyCredited) {
            // Coins were already added (e.g. by auto-check on BalancePage).
  ...
  Future<void> _creditCoinsFromTransaction(
    ...
    try {
      ...
      final currentBalance =
          prefs.getDouble('local_coins') ?? await HttpService().getLocalCoins();

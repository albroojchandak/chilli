...
import 'package:chilli/components/profile_card.dart';
import 'package:chilli/pages/calls_history_page.dart';
import 'package:chilli/pages/balance_page.dart';
import 'package:chilli/pages/profile_page.dart';
import 'package:chilli/core_services/cloud_database_service.dart';
import 'package:chilli/data_models/user_data.dart';
...
import 'package:chilli/components/call_request_dialog.dart';
import 'package:chilli/components/low_balance_dialog.dart';
import 'package:chilli/core_services/http_service.dart';
import 'package:chilli/core_services/realtime_db_service.dart';
import 'package:chilli/core_services/authentication_service.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:chilli/core_services/app_version_service.dart'; // ✅ Added AppVersionService
import 'package:chilli/core_services/app_rating_service.dart'; // ✅ Added AppRatingService
import 'package:chilli/helpers/profile_image_helper.dart'; // ✅ Added ProfileImageHelper
import 'package:chilli/helpers/profession_helper.dart'; // ✅ Added ProfessionHelper
...
class _MainPageState extends State<MainPage> with WidgetsBindingObserver {
  ...
  final HttpService _HttpService =
      HttpService(); // ✅ Added HttpService instance
  ...
  Stream<List<UserData>>? _usersStream;
  ...
  void initState() {
    ...
    AppRatingService.checkAndShowRating(context);
  ...
  /// ✅ Sync local coins to RTDB (Handled by HttpService)
    ...
    try {
      ...
      if (localData != null) {
        ...
        if (avatar == null || avatar.isEmpty) {
          ...
          avatar = ProfileImageHelper.getRandomAvatar(gender);
        ...
        if (career == null || career.isEmpty || career == 'Professional') {
          ...
          career = ProfessionHelper.getCareerForUser(localData['uid'] ?? '');
        ...
        final userData = UserData.fromMap(localData).copyWith(
          coins: localCoins,
                      ...
                      child: StreamBuilder<List<UserData>>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          */
                          ...
                          // ✅ Sort users: language→status→lastActive
                          users.sort((a, b) {
                            // Priority 1: Same language as current user
                            ...
                            // Priority 2: Status (Online/Active > Busy > Offline)
                          ...
                          return Column(
                            children: users.map((user) {
                              return ProfileCard(
                                ...
                                audioPrice:
                                    '${(HttpService.appConfig['male_audio_cost'] ?? 2.5) * 2}',
                                videoPrice:
                                    '${(HttpService.appConfig['male_video_cost'] ?? 2.5) * 2}',
                                isOnline:
      ...
      if (_currentCoins < 5) {
        ...
        if (mounted && !_isShowingLowBalanceDialog) {
          ...
          showDialog(
            ...
            builder: (dialogContext) => LowBalanceDialog(
  ...
  void _showCallRequestDialog(Map<String, dynamic> data, bool isVideoCall) {
    ...
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) => ModernCallRequestDialog(
  ...
  void _startCall(UserData targetUser, bool isVideoCall) async {
  ...
  void _startRandomCall(bool isVideoCall) async {
    ...
    try {
      ...
      final now = DateTime.now();
      final List<UserData> candidates = users.where((u) {
        // Basic checks
        final isNotMe = u.uid != currentUser.uid;
        final isOnline =
            u.status.toLowerCase() == 'online' ||
      ...
      // 4. Convert to Map list for VideoCallPage

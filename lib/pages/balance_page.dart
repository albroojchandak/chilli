...
import 'package:chilli/config/typography_styles.dart';
...
import 'package:chilli/pages/transactions_page.dart';
...
class _BalancePageState extends State<BalancePage>
  ...
  // Using ThemeColors for consistency, mapped to local variables for ease of porting
  final primaryColor = ThemeColors.primary; // Was Color(0xFF6A98F0)
  ...
  void initState() {
    ...
    _coinSubscription = HttpService.tokenStream.listen((newCoins) {
      if (mounted) {
        setState(() {
          currentCoins = newCoins;
        });
        print('💰 BalancePage: Coins updated to $newCoins');
      }
    });
  ...
  Future<void> _checkLastPendingPaymentOnStart() async {
    try {
      ...
      final mid = (lastUnresolved['mid']?.toString() ?? '').isNotEmpty
          ...
          : merchantId ??
                HttpService.appConfig['paygic_mid']?.toString().trim() ??
                '';
      final token = (lastUnresolved['token']?.toString() ?? '').isNotEmpty
          ? lastUnresolved['token'].toString()
          : paygicToken ??
                HttpService.appConfig['paygic_token']?.toString().trim() ??
                '';
      ...
      if (coinsToAdd > 0) {
        final currentBalance = await HttpService().getLocalCoins();
        final newBalance = currentBalance + coinsToAdd.toDouble();
        await prefs.setDouble('local_coins', newBalance);
        HttpService.broadcastTokenUpdate(newBalance.toInt());
        print(
  ...
  Future<void> _checkDailyBonusStatus() async {
    ...
    if (HttpService.appConfig['is_reward_enabled'] != true) {
    ...
    try {
      ...
      final DateTime serverNowRaw = await HttpService().getServerDate();
  ...
  Future<void> _claimDailyBonus() async {
    ...
    try {
      ...
      await HttpService().updateLocalCoins(bonusAmount);
      await HttpService().syncCoinsWithServer();
      ...
      // update Firestore (via AuthenticationService helper or direct if needed)
      ...
      final newCoins = await HttpService().getLocalCoins();
      HttpService.broadcastTokenUpdate(newCoins.toInt());
  ...
  Future<void> _loadUserData() async {
    ...
    if (HttpService.appConfig['paygic_mid'] == '') {
      await HttpService().fetchAppConfig();
    }

    if (mounted) {
      final config = HttpService.appConfig;
      ...
      final realBalance = await HttpService().getLocalCoins();
      if (mounted) {
        setState(() {
  ...
  Future<void> _submitWithdrawalRequest(double amount, String upi) async {
    ...
    try {
      ...
      final currentBalance = await HttpService().getLocalCoins();
      await HttpService().updateLocalCoins(currentBalance, isDeduction: true);
      await HttpService().syncCoinsWithServer(); // Sync to Firestore

      final newCoins = 0;
      HttpService.broadcastTokenUpdate(newCoins);
      ...
      final response = await http
          .post(
            ...
            body: jsonEncode({
              ...
              'redirect_URL': 'https://www.inflyratech.site/',
              'failed_URL': 'https://www.inflyratech.site/',
            }),
          )
          .timeout(const Duration(seconds: 15));
  ...
  Future<void> _handleSuccessfulPayment() async {
    ...
    HttpService.broadcastTokenUpdate(targetBalance);
  ...
  Future<void> _savePendingTransaction() async {
    ...
    final transactionData = {
      ...
      'refId': _currentRefId,
      'email': userEmail,
      // Store credentials so TransactionsPage can check status independently
      'mid': merchantId ?? '',
      'token': paygicToken ?? '',
    };
    await _saveLocalTransaction(transactionData);
                ...
                child: ClipRRect(
                  ...
                  child: BackdropFilter(
                    ...
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        // Header Row - Title and History Button
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            // Debug Add 1000 button
                            GestureDetector(
                              onTap: () async {
                                await HttpService().updateLocalCoins(1000);
                                await HttpService().syncCoinsWithServer();
                                _showToast(
                                  'Debug: Added 1000 coins',
                                  Colors.green,
                                );
                              },
                              child: Container(
                ...
                Container(
                  ...
                  child: Center(
                    child: Text(
                      '₹${package['price']}',
                      style: TextStyle(
                        color: isPopular ? Colors.white : ThemeColors.primary,
                        fontWeight: FontWeight.bold,
                        fontSize: 13,
                      ),
                    ),
            ...
            Positioned(
              ...
              child: Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: isSelected ? ThemeColors.primary : Colors.orange,
                    borderRadius: BorderRadius.circular(16),
                    boxShadow: [
                      BoxShadow(
                        color:
                            (isSelected ? ThemeColors.primary : Colors.orange)
                                .withOpacity(0.4),
                        blurRadius: 4,
